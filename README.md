...
`Polling 방식`으로 사용되는 두 API가 서버에서 `항상 DB를 조회`하고 있었습니다. 이들은 다른 API의 응답시간에 영향을 주었습니다. 인덱스를 추가해봤지만 여전히 해결되지는 않았습니다.
...
`주문 API`가 호출하는 `주문 생성 함수`는 복잡한 로직을 가지고 있었습니다. 돌아가기는 하지만 정상적으로 동작하는 API 였기에 바로 리팩토링할 수 없었습니다. 그래서 리팩토링 하기전에 테스트 코드를 짜보고자 했습니다.

만만치 않았습니다. 주문 API `테스트 코드를 작성`하면서 여러가지 `고민`들이 생겨났습니다. 여러 고민과 불편함을 마주치고 다시 리팩토링을 하길 결심했습니다.

테스트 코드를 쉽게 짜고자 다시 리팩토링하는 방향으로 돌아간 것이었습니다.
